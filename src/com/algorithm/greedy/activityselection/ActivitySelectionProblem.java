...
public class ActivitySelectionProblem {
  ...
  public static void main(String[] args) {
    List<Activity> activities = new ArrayList<>(Arrays.asList(new Activity(0, 0), // 0,0 is symbolic
        new Activity(1, 4), new Activity(3, 5), new Activity(0, 6), new Activity(5, 7), new Activity(3, 9), new Activity(5, 9),
        new Activity(6, 10), new Activity(8, 11), new Activity(8, 12), new Activity(2, 14), new Activity(12, 16)));
    // first step, sort by finish time
    Collections.sort(activities, Comparator.comparingInt(Activity::getFinish));

    List<Activity> selectedActivities = new ArrayList<>();
    // this is iterative greedy
  ...
  private static List<Activity> GREEDY_ACTIVITIY_SELECTOR(List<Activity> activities) {
    ...
    selectedActivity.add(activities.get(0));
    int k = 1;
  ...
  private static void RECURSIVE_ACTIVITY_SELECTOR(List<Activity> activities, int k, List<Activity> selectedActivities) {
    int m = k + 1;
